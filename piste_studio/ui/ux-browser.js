...
renderMedia=function(){
  ...
  if(libraryMode==='video'){
    ...
    list.forEach(m=>{
      ...
      e.innerHTML=`<div class="media-row-head"><b>${m.label}</b><small>${m.duration.toFixed(1)}s</small></div><div class="filmstrip" data-filmstrip="${m.id}">${m.filmstripUrl?`<img class="filmstrip-cache" src="${m.filmstripUrl}" alt="">`:''}${m.url?`<video class="filmstrip-skimmer" src="${m.url}" muted preload="metadata"></video>`:''}<div class="filmstrip-cells"><i></i><i></i><i></i><i></i><i></i></div>${typeof editorialRangeBands==='function'?editorialRangeBands(m):`<span class="favorite-band" style="left:${rangeL}%;width:${rangeW}%"></span><span class="reject-band" style="left:${rangeL}%;width:${rangeW}%"></span>`}<span class="range-band" style="left:${rangeL}%;width:${rangeW}%"></span></div><div class="media-meta"><div class="tag-row">${m.canon?'<span class="badge canon">CANON</span>':''}${m.safe?'<span class="badge safe">SAFE</span>':''}${m.favorite?'<span class="badge favorite">★ FAVORITE</span>':''}${m.analysis?.status==='READY'?'<span class="badge intelligence">ANALYSÉ</span>':''}${m.url?'<span class="badge local">LOCAL</span>':''}</div><span class="badge">S${m.spoiler||0}</span></div>`;
      ...
      strip.addEventListener('pointermove',ev=>{if(!m.url)return;strip.classList.add('scrubbing');const box=strip.getBoundingClientRect(),ratio=Math.max(0,Math.min(1,(ev.clientX-box.left)/box.width)),t=ratio*m.duration;const mini=strip.querySelector('video');try{if(mini)mini.currentTime=t}catch(_){}if(selectedMedia===m.id)showSourceMedia(m,t,false)});strip.addEventListener('pointerleave',()=>strip.classList.remove('scrubbing'));
      root.appendChild(e)
    })
  }else{
