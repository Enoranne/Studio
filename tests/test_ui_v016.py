...
def test_v016_ui_structure_and_assets(tmp_path):
    ...
    assert "PISTE Studio — Local App v0.16" in html
    assert 'id="editorialDrawer"' in html
    assert 'id="markerComposer"' in html
    assert 'id="addMarkerBtn"' in html
...
def test_v016_ux_contains_editorial_intelligence_patterns():
    ux = "".join(
    ...
    assert "editorialRangeBands" in ux
    assert "openSourceSelector" in ux
    assert "human" not in ux.lower() or "validation" in ux.lower()
    assert "openMarkerComposer" in ux
    assert "renderEditorialMarkers" in ux
